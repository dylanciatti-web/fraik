...
- **Test final 15 € lancé par Dylan (13/07 soir) avec la créa V8** (`fraik_meta_ad_v8_final.mp4`). Règle de décision déjà actée (voir Décisions récentes) : CTR ok + 0 ATC → on arrête Fraïk, pas de nouveau fix technique ; ATC mais 0 achat → auditer confiance/paiement ; achat → stabiliser. **Juger sur les commandes Shopify réelles (`list-orders`), pas sur les métriques Ads Manager.** Résultat pas encore connu à cette mise à jour — prochaine étape : vérifier `list-orders` et les ATC/InitiateCheckout du dataset FRAÎK une fois le palier de 15 € atteint.
- **Créa vidéo V8 montée pour le test final 15 € (13/07 soir) :** Codex avait proposé une V8 minimale (produit réel, brume visible, personne qui réagit, pas de scène IA) après avoir jugé AD02 (image statique, produit absent) et la vidéo v7 (rupture de qualité entre l'ouverture IA premium et le plan produit) inadaptées pour un test propre. Montée à partir des deux rushes UGC fournis par Dylan (variante **NOIR**, confirmée vendue et active côté Shopify avant tout montage — condition impérative posée par Dylan). Montage 1080×1920, 30 fps, 12,00 s exactes, H.264 yuv420p +faststart, sans audio, respectant le découpage/habillage texte du brief. **Écart assumé et validé par Dylan par rapport au brief d'origine :** le plan "produit frontal net, zoom lent" prévu sur B[3,00–6,75s] n'était en réalité stable que ~1,3 s avant qu'une main ne soulève le produit vers une autre table — repéré en scannant les rushes image par image avant montage. Après validation de Dylan (scan d'abord, freeze en dernier recours ≤1 s), le segment final combine : plan produit réel (1,4 s) → freeze court (0,9 s, sous le plafond d'1 s) → retour sur le plan visage+produit déjà utilisé plus tôt dans la vidéo (continuité réelle, pas de nouveau rush), avec le zoom 100→108 % appliqué en continu sur l'ensemble pour lisser la coupure. **Deux corrections demandées par Dylan après relecture, appliquées (13/07 soir) :** réencodage plus léger (CRF 23, maxrate 5M, bufsize 10M → 7,5 Mo au lieu de ~25 Mo/16,5 Mbps) ; texte de l'écran 5 remplacé (`PAS UNE CLIM. / UN SOULAGEMENT LOCAL.` → `FRAÎCHEUR LOCALISÉE.`, Dylan jugeant que répéter la négation juste avant le prix transformait le CTA en objection). Fichier final validé par Dylan : `/Users/Dylan/Downloads/Fraïk/fraik_meta_ad_v8_final.mp4` (+ 3 captures de contrôle à 0,75/6,5/10,8 s dans le même dossier). **Mise en ligne Meta en cours de préparation par Dylan :** dupliquer AD01 vidéo, remplacer son média par V8, garder une seule pub active ; la V8 9:16 ne doit être diffusée que sur Reels et Stories (Instagram/Facebook), pas dans les fils ni Explorer, qui plafonnent au ratio 4:5. Attendre la fin du traitement média et vérifier les aperçus Reels + Stories avant publication ; budget total plafonné à 15 €, aucun autre changement pendant le test.
- **Campagne Meta Ads coupée par Dylan (13/07 soir) :** palier des 15€ supplémentaires atteint sur AD02 → **0 ATC, 0 InitiateCheckout, 0 achat** confirmé (Events Manager à 0/0, `list-orders` Shopify à 0 total). Ça correspond exactement à la branche "couper" du plan (tâche 2 ci-dessous, maintenant traitée). Diagnostic : CTR/CPC d'AD02 étaient bons mais quasi aucun clic ne se transformait en ATC — le point de rupture est très en amont (avant même la page), cohérent avec les bounces Clarity à 0-2 sec. **Ne pas relancer de budget avant d'avoir laissé le temps aux fixes de poids de page ci-dessous de produire un effet, et sans nouvelle validation explicite de Dylan.**
- **Vidéo hero réencodée et allégée (13/07 soir) :** poids identifié comme suspect technique principal des bounces 0-2 sec (3,19 Mo, autoplay, premier élément visible de la page, sur potentiellement de la 4G en pleine canicule). Réencodée localement en 480×480 puis uploadée via l'API Shopify (`stagedUploadsCreate` + `productCreateMedia`) ; Shopify a retraité le fichier à **860 Ko (0,9 Mbps), soit −73% vs l'original**, avec un seul rendu mp4 exposé côté Liquid (donc plus d'ambiguïté sur la source servie). Le template ne boucle plus sur toutes les sources vidéo (comportement non déterministe selon navigateur) : il sélectionne désormais explicitement la source mp4 la plus légère (`fraik-product.liquid`, commit `48cba11`). Anciennes vidéos (1080p 3,19 Mo + un rendu intermédiaire 720p 1,54 Mo généré en cours de route) **supprimées du produit après validation explicite de Dylan** — original sauvegardé localement dans le scratchpad de session si besoin de restauration. Vérifié en live : poids réel 860 Ko, rendu mobile identique visuellement, galerie/sélecteur de variante toujours fonctionnels. **Effet secondaire utile :** en testant, seule exception console détectée = un throw interne au script tiers Microsoft Clarity (`clarity.js`, "Cannot read properties of null (reading 'sequence')"), pas de notre code — piste probable pour l'erreur "undefined is not an object" de la tâche 4 (même famille d'erreur, formulation différente selon navigateur ; pas un bug Fraïk, rien à corriger côté thème).
...
2. ~~Laisser uniquement AD02 tester...~~ **Traitée (13/07 soir) :** palier atteint, 0 ATC confirmé, Dylan a coupé la campagne. Prochaine étape : décider quand/si relancer un test, après un temps d'observation post-fixes de poids de page (vidéo notamment) — pas de nouveau budget sans validation explicite de Dylan.
